...
| `--axis` | 整数晶轴，默认 `110`；负数或多位数用引号和空格，例如 `"1 -1 3"` |
| `--angle` | 参考错取向角 0–90°；未指定时 `110` 选 Σ9、`100` 选 Σ5，其他轴选最低 Σ 预设，无预设则 0° |
| `--lattice-constant` | 参考晶格常数 a₀，默认 3.52 Å；不会改变图中归一化坐标 |
| `--width`、`--height` | 基础视野尺寸，默认 12、9，单位 a₀ |
| `--marker-size` | 原子标记尺寸参数，默认 32 |
| `--view-scale` | 初始视野倍率，0.1–5，默认 1 |
| `--workers` | 计算进程数，默认最多 4 个并受可用 CPU 数限制；1 禁用进程池 |
| `--save` | 导出当前初始图为 PNG 后退出 |
...
GUI 中的 `Export plot as PNG…` 导出当前状态，包含显示旋转和标注。

### 交互与快捷键

| 操作 | 效果 |
| --- | --- |
| 拖动 / 滚轮 | 平移 / 缩放，不清除未完成的选择 |
| `R` | 选择 B1、B2 定义 GB 参考线；左右侧相对 B1→B2 定义 |
| `V` | 选择 P1、P2 测量向量 |
| `M` | 选择四个同层 common sites 定义手动胞 |
| `C` | 居中视野 |
| `F` | 显示两晶粒 GB 线两侧，Layers 过滤仍有效 |
| `1` / `2` | G1 左/G2 右，或 G1 右/G2 左 |
| `Esc` | 返回空闲模式，暂停当前选择 |
...
默认打开 Orientation，切换页签时按当前内容调整高度。Layers 按当前观察轴列出
G1/G2 各自的轴向层，带对应颜色与形状的图标，每个晶粒的每一层均可独立开关；
隐藏的原子不参与鼠标拾取，该层两晶粒都可见时才显示对应 CSL/Local Near-CSL 标记。
手动胞一旦选定后仍只按其顶点所属层计数，不受随后显示层开关影响。
`Show automatic cell` 默认关闭。
...
自动周期框由 `Show automatic cell` 独立控制。
`MANUAL COMMON CELL` 面板使用步骤：
...
- 四点形成平行四边形时，采用半开（half-open）约定计数，并在图中优先显示它。
  定义为 `C1 + u(C2−C1) + v(C4−C1)`，`0 ≤ u,v < 1`；排除两条上界边，
  避免重复平铺时反复计入边界原子。非平行四边形只给胞内/边界/闭合计数。
- 图中使用直观的 `40 atoms · 34 inside + 5 edge + 1 corner` 等表示。
  这里的 5 个边原子和 1 个顶点是半开约定保留的原子，并非整个闭合轮廓上的全部边界原子。
  `◇ layer` 表示菱形层，其他层使用相应的符号；不是只统计菱形 CSL 点。
...
- 成功应用后，手动胞面板下方出现独立的只读应变详情框，可滚动查看和复制。
  其中分别列出 G1/G2 的主伸长、带正负号的主应变、极分解旋转、`F/R/U` 矩阵、
  分析坐标及参考立方晶粒坐标下的 Green–Lagrange `E`、均匀平移和面积变化；
  同时显示输入上限、参考/极分解错取向角、四对顶点的对齐残差和共同胞矢量。
  应变百分数与无量纲张量明确区分，所有物理量不包含 `Display rotation`。
  未应用、应用失败、恢复或退出该功能时详情框隐藏且清空，不占空白空间；
  平移、缩放、显示旋转和更换 worker 不会清空内容或重置阅读位置。
